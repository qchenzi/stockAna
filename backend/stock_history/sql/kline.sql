-- 计算均线
-- 查询某只股票在某日期的 5 日、10 日和 200 日移动平均线
WITH ma_calculation AS (
    SELECT stock_code,
           trade_date,
           AVG(close_price) OVER (PARTITION BY stock_code ORDER BY trade_date ROWS 4 PRECEDING) AS ma_5,   -- 5日均线
           AVG(close_price) OVER (PARTITION BY stock_code ORDER BY trade_date ROWS 9 PRECEDING) AS ma_10,  -- 10日均线
           AVG(close_price) OVER (PARTITION BY stock_code ORDER BY trade_date ROWS 199 PRECEDING) AS ma_200 -- 200日均线
    FROM stock_historical_quotes
    WHERE stock_code = '600519' -- 指定股票代码
)
SELECT *
FROM ma_calculation
WHERE trade_date = '2023-12-20'; -- 筛选目标日期
