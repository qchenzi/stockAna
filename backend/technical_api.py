...
@technical_bp.route('/recommendations', methods=['GET'])
def get_stock_recommendations():
    """获取股票推荐列表"""
    try:
        with engine.connect() as conn:
            # 1. 获取最新交易日期
            latest_date_sql = "SELECT MAX(trade_date) as latest_date FROM stock_historical_quotes"
            latest_date = conn.execute(text(latest_date_sql)).scalar()
            ...
            # 2. 查询推荐数据
            fetch_sql = """
            SELECT stock_code, stock_name, industry, current_price, 
                   total_score, recommendation_level, reasons
            FROM stock_recommendations
            WHERE recommend_date = :date
            ORDER BY total_score DESC
            """
            results = conn.execute(text(fetch_sql), {'date': latest_date}).fetchall()

            # 3. 如果没有推荐数据，生成新的推荐
            if not results:
                from stock_recommendation_generator import generate_recommendations
                generate_recommendations(20)  # 默认生成20条推荐
                # 重新查询推荐数据
                results = conn.execute(text(fetch_sql), {'date': latest_date}).fetchall()
